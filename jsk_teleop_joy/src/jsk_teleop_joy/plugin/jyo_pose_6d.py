...
class JyoPose6D(RVizViewController):
  '''
Usage:
...
set_pose [String, default: set_pose]: topic name for setting pose by topic
  '''
  #def __init__(self, name='JyoPose6D', publish_pose=True):
  def __init__(self, name, args):
    ...
    if self.publish_pose:
      ...
      self.target_pub = rospy.Publisher(self.getArg('target_pos', 'target_pos'),
    ...
    if not status.R3:
      ...
      if status.right:
        ...
        if status.square:
          ...
          roll = roll - DTHETA
    if not status.R3 and status.circle and not latest.circle and self.publish_pose:
      self.publishText(new_pose.pose.position)
  ...
  def publishText(self, pos, atop=10, aleft=10):
    target_pos = OverlayText()
    target_pos.top = atop
    target_pos.left = aleft
    target_pos.width = 256
    target_pos.height = 160

    color1 = ColorRGBA(0.0, 0.0, 0.0, 0.8)
    color2 = ColorRGBA(25.0/255.0, 255.0/255.0, 240.0/255.0, 0.8)

    target_pos.bg_color=color1;
    target_pos.fg_color=color2;
    target_pos.text_size = 16
    target_pos.line_width = 1
    target_pos.font = 'Ubuntu'

    containt = ('pos.x:\n' + str(pos.x) +
                '\npos.y:\n' + str(pos.y) +
                '\npos.z:\n' + str(pos.z))
    target_pos.text = containt
